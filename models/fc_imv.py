...
class FC_Marginal(nn.Module):
    def __init__(self, input_size: int, hidden_sizes: List[int], z_dim: int, prediction_dim: int):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        for sz in range(1, len(self.hidden_sizes)):
            h = F.relu(getattr(self, f'fc{sz+1}')(h))
...
class JointVAE(nn.Module):
    def __init__(self, marginal_models: List[FC_Marginal], hidden_dim = 128):
        super().__init__()
    ...
    def loss(self, y, yhat, poe_dist, yhats, dists, var_beta=1.):
        ...
        product_loss = var_loss(y, yhat, poe_dist, var_beta=var_beta)

        # compute the variational loss for each marginal model
        var_losses = [var_loss(y, yhat, dist) for yhat, dist in zip(yhats, dists)]
    
        return product_loss + sum(var_losses)
    
def var_loss(y, yhat, dist, var_beta=1.):
    ...
    ce = F.cross_entropy(yhat, y, reduction='none').sum(dim=-1)
