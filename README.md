...
I also include an implementation of a simple multi-view MLP in `models/multi_mlp.py`.  It is very similar to the DeepIMV architecture, but does not use any variational inference.  Instead it simply computes an average of the latent representations of the different views, and then uses that as input to the the final MLP.  An example of running this model is in multiview_mlp_example.ipynb.
